...
def resolve_identity(platform: str, user_id: str) -> Dict[str, Any]:
    ...
    # CLI/SSH fallback — 无 user_id (None) 时默认走 cli_default 身份
    if platform == "cli" and user_id is None:
        for name, identity in identities.items():
            if identity.get("cli_default"):
                return {
                    "name": name,
                    "role": identity.get("role", "user"),
                    "notes": identity.get("notes", ""),
    ...
    # Guest workspace — create on the fly for unknown users
    guest_ws = f"workspaces/guest_{user_id}" if user_id is not None else "workspaces/guest"
